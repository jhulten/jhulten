# Jeffrey Hulten

> Builder obsessed with resilient software, thoughtful teams, and products that make people faster.

Senior Systems Engineer @ Getty Images, currently shepherding Kubernetes-based artifact management and build tooling.

## Snapshot
- 🛠️ Leads Getty Images' Kubernetes and build-tooling platform workstreams in AWS.
- 🎯 Steers ambitious ideas toward reliable, maintainable implementations balancing speed with resilience.
- 🧠 Blends systems thinking, AI curiosity, and product empathy to keep solutions practical.
- 🕹️ Enjoys hands-on prototyping just as much as shaping long-term engineering strategy.
- 🤝 Champions human collaboration, deep code reviews, and observability-first cultures.

## Work I Gravitate Toward
| Area | What excites me |
| --- | --- |
| Distributed & event-driven systems | Turning messy real-world signals into trustworthy, low-latency decisions. |
| Developer experience | Building tooling, guidelines, and automation that let teams move confidently. |
| Kubernetes platforms & cloud build systems | Operating clusters, supply chains, and delivery guardrails that keep releases safe and fast. |

## Operating Principles
1. **Bias for observability** – instrument early so every release comes with data, not suspense.
2. **Design for change** – modular interfaces, generous test coverage, and CI/CD that keeps refactors safe.
3. **Document the why** – decisions, playbooks, and architecture notes live beside the code.

## Current Focus
- ☁️ Deploying build tools like Gitlab Enterprise and Artifactory on AWS EKS with sane defaults and guardrails.
- 📦 Hardening platform foundations: API boundaries, golden paths, and progressive delivery.

## Toolbox at a Glance
- **Systems**: distributed services, streaming pipelines, edge & IoT orchestration.
- **Platform**: Kubernetes (EKS), build systems, CI/CD, IaC, observability, incident response, SLO automation.

## Let's Collaborate
- 🔭 Open to chatting about platform modernization or developer productivity bets.
- 📫 Drop a note via the contact channels below (or suggest your favorite community) and let’s brainstorm.

